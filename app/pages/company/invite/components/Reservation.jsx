...
import ReservationCalendar from '../../../../../components/calendar/components/ReservationCalendar.jsx'
...
let Reservation = React.createClass({
  ...
  _handleClick:function(){
      //点击手动预约按钮
      $("input[name=btn-res]").click(function() {
          //模态框外部不可点击
          $("div[name=CalendarModal]").modal({
             backdrop: false,
             keyboard: false,
             show:true
          });
      ...
      $("button[name=close1]").click(function() {
          $("div[name=CalendarModal]").modal('hide');
      });
  },
  render: function () {
    return (
      <div>
          {/* 手动预约按钮 */}
          <input type="button" name="btn-res" className="btn btn-primary" value="手动预约"/>
          {/* 会议室预约模态框 */}
          <div className="modal fade" name="CalendarModal" tabIndex="-1" role="dialog" aria-labelledby="CalendarModalabel" aria-hidden="true">
              <div className="modal-dialog" style={{width:"90%"}}>
                  <div className="modal-content">
                      <div className="modal-header">
                          <button type="button" className="close" name="close1" aria-hidden="true">
                              &times;
                          </button>
                      ...
                      <div className="modal-body">
                          <WidgetGrid>
                              {/* 调用日历组件 */}
                              <ReservationCalendar reservation={this.state.reservation} maxId={this.state.maxId}/>
                          </WidgetGrid>
                      </div>
                  </div>
              </div>
