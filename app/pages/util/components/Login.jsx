...
let Login = React.createClass({
    ...
    componentDidMount: function() {
        ...
        // 2. 监听公司列表获取事件，当获取到公司列表时，执行_onSearchCompanyDone()
        this.unsubscribeCompany = CompanyStore.listen(this._onSearchCompanyDone);
        this.unsubscribeLogin = LoginStore.listen(this._onLoginSuccess);
        // 3. 获取公司列表
        LoginAction.searchCompany();
    },
    ...
    _formValidate: function() {
        ...
        // 当用户类型选择了 公司管理员《 公司部门领导，公司员工或公司接待 之一时
        // 要求选择相应公司名称
        if ($.inArray($("#sel-user-type").val(), ['agent', 'platform-admin']) == -1 &&
            $("#sel-company").val() == "") {
            $("#div-company").addClass("vs-invalid").removeClass("vs-valid");
            pass = false;
        }
        return pass;
    },
    ...
    _registerEventHandler: function() {
        ...
        $("#sel-user-type").change(function() {
            ...
            // 根据当前选择用户类型决定是否显示公司选择框
            if ($.inArray($(this).val(),
                  ['company-admin', 'company-depart-leader', 'company-receptionist', 'company-staff']) != -1) {
            ...
            if ($("#user-type").val() != "") {
                $("#div-user-type").addClass("vs-valid").removeClass("vs-invalid");
            }
        }).change();

        $("#sel-company").change(function() {
            $("#div-company").addClass("vs-valid").removeClass("vs-invalid");
        });

        // 当表单提交时触发事件
        $("#btn-login").click(function(event) {

          // VsUtil.Confirm(UtilMsg.Msg001,me._OK,me._NO);
            // 防止表单默认提交
            ...
            // 公司ID
            let companyId = $("#sel-company").val();
            // 登录用户名
            let loginName = $("#txt-login-name").val().trim();
            // 登录密码
    ...
    // 当获取到公司列表触发
    // TODO: 当前做法不适合根据输入动态获取匹配列表的场合
    _onSearchCompanyDone: function(data) {

        data.companies.forEach(function(company, index) {
            $("#sel-company").append(new Option(company.companyName, company.companyId, false, false)).trigger('change');
        })

        $("#sel-user-type").change();
    },
    _onLoginSuccess: function(result) {
      ...
      let realnameMapper = {
        'agent': 'agentName',
        'company-admin': 'companyadminName',
        'company-depart-leader': 'staffName',
        'company-receptionist': 'staffName',
        'company-staff': 'staffName',
      ...
      switch (result.data.bizCode) {
        ...
        case config.API_BIZ_CODE.BIZ_CODE_RECORD_0:
          VsUtil.ToastrError({content: UtilMsg.Msg001});
          break;
        case config.API_BIZ_CODE.BIZ_CODE_LOGIN_AUTH_FAILED:
          VsUtil.ToastrError({content: UtilMsg.Msg002});
          break;
      }
    },
    // 画面渲染
                                        ...
                                        <fieldset>
                                            <section>
                                                ...
                                                <label className="select"> <i className="icon-append fa fa-user"/>
                                                    <div id='div-user-type'>
                                                        <Select2 id="sel-user-type" multiple={false} style={{width: '100%'}} data-select-search="false" data-language="zh-CN" data-minimum-results-for-search="Infinity" data-placeholder="请选择您的登录用户类型"
                                                           name="usertype" defaultValue="" data-smart-validate-input="" data-required="" data-message-required="">
                                                            <option value=""></option>
                                            ...
                                            <section id="section-company">
                                              <label className="label">公司名称</label>
                                                <label className="select"> <i className="icon-append fa fa-user"/>
                                                <div id="div-company">
                                                  <Select2 multiple={false} style={{width: '100%'}} data-select-search="true" data-language="zh-CN" data-placeholder="请选择您的公司"
                                                     id="sel-company" defaultValue="" name="company" >
                                                    <option value=""></option>
                                                  </Select2>
                                                </div>
                                              </label>
